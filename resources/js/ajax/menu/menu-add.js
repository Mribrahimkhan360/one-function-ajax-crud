$(document).ready(function () {
    ...
    // ==========================================================
    //  1. CSRF TOKEN SETUP — Protect all AJAX requests
    // ==========================================================
    ...
    // ==========================================================
    //  4. UPDATE ICON - Update icon preview on change
    //  preview on selection
    // ==========================================================
    function UpdateIconPreview() {
        ...
        select.on("change", function () {
            const val = $(this).val();
            preview.html(val ? `<i class="${val}"></i>` : "");
        });
    }
    ...
    // ==========================================================
    //  5. PARENT MENU - Fetch menus from server
    // ==========================================================
    function loadMenus() {
        $.ajax({
            url: '/menus/list',    // Laravel route URL
            type: "GET",           // Request type
            dataType: "json",      // Expect JSON response
            success: function (data) {

                const allMenus = data.map(m => m.title);

                menuList.empty(); // Clear old options

                menuList.append('<option value="">No Parent</option>');
                allMenus.forEach(menu => {
                    menuList.append(`<option value="${menu}">${menu}</option>`);
                });
            },
            error: function (err) {
                console.log(err);
    ...
    // ==========================================================
    //  6. FETCH SIDE MENUS & DROPDOWN - Fetch and display
    //  side menus. Load menus and update sidebar & dropdown
    // ==========================================================
    function menuFetch() {
        $.ajax({
            url: '/menus/list', // Laravel route
            method: 'GET',
            dataType: 'json',
            success: function (data) {
                const sidebar = $("#side-menus");
                const dropdown = $("#menuList");
                const input = $("#menuTitle");

                // Database menus
                const menus = data.map(m => ({title: m.title, icon: m.icon})); // "bx bx-menu"
                        ...
                        <ul class="metismenu list-unstyled" id="side-menu">
                            <li>
                                <a href="javascript: void(0);" class="has-arrow waves-effect">
                                    <i class="${m.icon}"></i>
                                    <span key="t-dashboards">${m.title}</span>
                                </a>
                ...
                $('#side-menus .metismenu').metisMenu();

                // Update dropdown options
                function updateDropdown(input) {
                    const val = input.val().trim();
                    // console.log(val);
                    dropdown.empty().append('<option value="">No Parent</option>');

                    // Add DB Menus
                    menus.forEach(m => dropdown.append(`
                         <option value="${m.title}">${m.title}</option>
                    `));

                    // Add input if not already present
                    if (val && !menus.some(m => m.title == val)) {
                        dropdown.prepend(`<option value="${val}">${val}</option>`)
                    }
                }

                // Real-time update on input
                input.off('input').on('input', updateDropdown);
    ...
    // ==========================================================
    //  7. UPDATE MENU LIST - Update menu list on input change
    // ==========================================================


    // ==========================================================
    //  8. STORE MENUS - Handle form submission to store menus
    // ==========================================================
    function menusStore() {
        $("#createMenuForm").on("submit", function (e) {
            ...
            const parent = $("#menuList").val();
            const route = $("#routeInput").val();
            const type = $("input[name='typeRadio']:checked").val();
            const status = $("#statusCheck").is(":checked") ? 1 : 0;
            ...
            $.ajax({
                ...
                type: "POST",
                data: {
                    title, icon, parent, route, type, status
                },
                success: function (res) {
                    ...
                    $("input[name='typeRadio'][value='Dropdown']").prop("checked", true);
                    loadMenus();
                    menuFetch();
                },
                error: function (err) {
